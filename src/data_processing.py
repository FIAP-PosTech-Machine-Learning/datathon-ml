...
# Para stemming ou lematização
from nltk.stem import PorterStemmer, WordNetLemmatizer
# Para vetorização de texto
from sklearn.feature_extraction.text import TfidfVectorizer
# Para calcular similaridade de cosseno
from sklearn.metrics.pairwise import cosine_similarity
# Para codificação de variáveis categóricas
from sklearn.preprocessing import LabelEncoder, OneHotEncoder
# Para tratamento de valores faltantes
from sklearn.impute import SimpleImputer
# Para aplicar diferentes transformações a diferentes colunas
from sklearn.compose import ColumnTransformer
# Para orquestrar as etapas de pré-processamento
from sklearn.pipeline import Pipeline
import numpy as np  # Para operações numéricas
import joblib  # Importar joblib para salvar o vetorizador
...
# data_path agora será o caminho absoluto
def load_and_combine_data(data_path):
    ...
    print(f"DEBUG: jobs_df shape: {jobs_df.shape}")  # LINHA DE DEBUG
    print("DEBUG: jobs_df head:\n", jobs_df.head())  # LINHA DE DEBUG
        ...
        if 'prospects' in job_info and isinstance(job_info['prospects'], list):
            for prospect in job_info['prospects']:
                ...
                prospect_record.update(
                    {k: v for k, v in job_info.items() if k != 'prospects'})
    ...
    print(f"DEBUG: prospects_df shape: {prospects_df.shape}")  # LINHA DE DEBUG
    print("DEBUG: prospects_df head:\n", prospects_df.head())  # LINHA DE DEBUG
    ...
    # LINHA DE DEBUG
    print(f"DEBUG: applicants_df shape: {applicants_df.shape}")
    print("DEBUG: applicants_df head:\n",
          applicants_df.head())  # LINHA DE DEBUG
    ...
    def flatten_json_column_local(df_to_flatten, column_name_to_flatten):
        flattened_data = pd.json_normalize(
            df_to_flatten[column_name_to_flatten])
        flattened_data.columns = [
            f"{column_name_to_flatten}_{col}" for col in flattened_data.columns]
    ...
    if 'job_id' not in jobs_df_flat.columns:
    ...
    print(f"DEBUG: jobs_df_flat shape: {jobs_df_flat.shape}")  # LINHA DE DEBUG
    print("DEBUG: jobs_df_flat head:\n", jobs_df_flat.head())  # LINHA DE DEBUG

    applicants_df_flat = applicants_df.copy()
    nested_applicant_cols = ['infos_basicas', 'informacoes_pessoais',
    ...
    for col in nested_applicant_cols:
        if col in applicants_df_flat.columns:
            if applicants_df_flat[col].apply(lambda x: isinstance(x, dict)).any():
                applicants_df_flat = flatten_json_column_local(
                    applicants_df_flat, col)
    # LINHA DE DEBUG
    print(f"DEBUG: applicants_df_flat shape: {applicants_df_flat.shape}")
    print("DEBUG: applicants_df_flat head:\n",
          applicants_df_flat.head())  # LINHA DE DEBUG
    ...
    print(f"DEBUG: matches_df shape: {matches_df.shape}")  # LINHA DE DEBUG
    print("DEBUG: matches_df head:\n", matches_df.head())  # LINHA DE DEBUG

    # Esta é a linha que falhou na execução anterior se 'final_df' não foi atribuído
    final_df = pd.merge(matches_df, jobs_df_flat,
                        on='job_id',
                        how='left', suffixes=('_match', '_job'))

    # Se você chegar até aqui sem erro, as merges iniciais foram bem-sucedidas.
...
# Adicionado models_path como parâmetro
def create_preprocessing_pipeline(df, models_path):
    """
    ...
    Args:
        ...
        models_path (str): Caminho absoluto para a pasta onde os modelos devem ser salvos.
    ...
    df_processed['is_successful_match'] = (
        df_processed['situacao_candidado'] == 'Contratado pela Decision').astype(int)

    # Separar X e y
    X = df_processed.drop(
        columns=['is_successful_match', 'situacao_candidado'], errors='ignore')
    ...
    for col in text_cols_for_processing:
        if col in X.columns:
            ...
            print(
                f"Aviso: Coluna de texto '{col}' não encontrada. Criando coluna vazia.")
            X[f'{col}_processed'] = ""
    ...
    for col in level_cols:
        if col in X.columns:
            X[f'{col}_encoded'] = X[col].fillna('').astype(
                str).map(nivel_mapping).fillna(-1).astype(float)
        else:
            X[f'{col}_encoded'] = -1.0

    # 4. Features de Match de Nível
    X['match_nivel_ingles'] = (X['formacao_e_idiomas_nivel_ingles_encoded']
                               >= X['perfil_vaga_nivel_ingles_encoded']).astype(int)
    X['match_nivel_profissional'] = (X['informacoes_profissionais_nivel_profissional_encoded']
                                     >= X['perfil_vaga_nivel_profissional_encoded']).astype(int)

    # 5. Extração de features de Remuneração e Faixa Etária
    if 'informacoes_profissionais_remuneracao' in X.columns:
        X['informacoes_profissionais_remuneracao_cleaned'] = X['informacoes_profissionais_remuneracao'].fillna(
            '').astype(str).str.replace(',', '.').str.extract(r'(\d+\.?\d*)').astype(float)
    else:
        X['informacoes_profissionais_remuneracao_cleaned'] = np.nan
    if 'perfil_vaga_faixa_etaria' in X.columns:
        X['perfil_vaga_faixa_etaria_min'] = X['perfil_vaga_faixa_etaria'].fillna(
            '').astype(str).str.extract(r'De:\s*(\d+)').astype(float)
        X['perfil_vaga_faixa_etaria_max'] = X['perfil_vaga_faixa_etaria'].fillna(
            '').astype(str).str.extract(r'Até:\s*(\d+)').astype(float)
        X['perfil_vaga_faixa_etaria_mid'] = X[[
    ...
    if 'informacoes_basicas_data_requicisao' in X.columns:
        X['vaga_idade_dias'] = (pd.to_datetime('today') - pd.to_datetime(
            X['informacoes_basicas_data_requicisao'], errors='coerce', dayfirst=True)).dt.days.fillna(-1)
    else:
        X['vaga_idade_dias'] = -1.0
    if 'data_candidatura' in X.columns:
        X['dias_desde_candidatura'] = (pd.to_datetime('today') - pd.to_datetime(
            X['data_candidatura'], errors='coerce', dayfirst=True)).dt.days.fillna(-1)
    else:
        X['dias_desde_candidatura'] = -1.0

    # --- 7. TF-IDF e Similaridade ---
    ...
    tfidf_vectorizer = TfidfVectorizer(
        max_features=2000, stop_words=list(STOPWORDS_PT))
    tfidf_vectorizer.fit(corpus)

    # --- ADICIONE ESTA LINHA PARA SALVAR O TFIDF VECTORIZER ---
    # Usa o 'models_path' recebido como parâmetro para salvar de forma robusta
    joblib.dump(tfidf_vectorizer, os.path.join(
        models_path, 'tfidf_vectorizer.pkl'))
    print(
        f"\nTF-IDF Vectorizer salvo em: {os.path.join(models_path, 'tfidf_vectorizer.pkl')}")
    # --- FIM DA ADIÇÃO ---
    ...
    numerical_features_final = [
        col for col in numerical_features_final if col in X.columns]
    categorical_onehot_features_final = [
        col for col in categorical_onehot_features_final if col in X.columns]

    print(
        f"Colunas numéricas finais para o pipeline: {numerical_features_final}")
    print(
        f"Colunas categóricas One-Hot finais para o pipeline: {categorical_onehot_features_final}")
    ...
    preprocessor = ColumnTransformer(
        transformers=[
            ...
            ('cat', OneHotEncoder(handle_unknown='ignore',
             sparse_output=False), categorical_onehot_features_final)
        ],
        remainder='drop'
    )
    ...
    X_processed = pd.DataFrame(
        X_processed_array, columns=feature_names, index=X.index)

    print(
        f"Pré-processamento concluído. Shape do DataFrame processado: {X_processed.shape}")
    return X_processed, y, preprocessing_pipeline


# --- Exemplo de Uso (para testar o script localmente) ---
# O bloco if __name__ == "__main__": não precisa ser alterado, pois ele
# não é chamado quando o script é importado por model_training_adjusted.py
if __name__ == "__main__":
    # Define o caminho para os dados (assumindo que você está executando de DATATHON/src/)
    # Ou ajuste para o seu ambiente de teste local
    base_data_path = '../data/'
    ...
    # Para testes locais, você pode passar um caminho temporário ou mockar models_path
    # Exemplo: create_preprocessing_pipeline(final_df_loaded.copy(), models_path='../models_temp/')
    # Certifique-se de criar models_temp antes de rodar este bloco de teste local.
    # Por enquanto, vou remover o argumento models_path daqui, pois este bloco é apenas para teste local
    # e o comportamento 'real' de salvamento está na chamada de model_training_adjusted.py
    X_processed_df, y_target, pipeline_trained = create_preprocessing_pipeline(
        final_df_loaded.copy(), models_path='.')  # Ou outro caminho temporário/mock
    # Se você quiser testar o salvamento de tfidf_vectorizer localmente a partir deste script,
    # você precisaria de um diretório 'models' dentro do 'src' ou ajustar o models_path.
    # Para a execução principal via model_training_adjusted.py, o que fizemos acima já resolve.
